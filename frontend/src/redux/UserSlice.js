...
export const UserWithBorrowBookThunk = createAsyncThunk("UserWithBorrowBookThunk", async (limit)=>{
    const userwithBorrowedBookResponse = await axios.get(`${import.meta.env.VITE_BASE_URL}/user/system/withborrowedbooks?limit=${limit}`);
...
export const UserSliceThunk = createAsyncThunk("UserSliceThunk", async ()=>{
    const totalUserResponse = await axios.get(`${import.meta.env.VITE_BASE_URL}/user/system/total`);
    const activeUserResponse = await axios.get(`${import.meta.env.VITE_BASE_URL}/user/system/active?day=7`);
    const newUserResponse = await axios.get(`${import.meta.env.VITE_BASE_URL}/user/system/new?day=7`);
